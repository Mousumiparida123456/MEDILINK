...
export function Features() {
  const features = [
    { icon: <Search className="h-6 w-6" />, title: "Smart Medicine Search", desc: "Instantly find medicines by brand, generic name, or symptoms across local pharmacies.", path: "/search" },
    { icon: <MapPin className="h-6 w-6" />, title: "Live Pharmacy Map", desc: "Discover nearby pharmacies using real-time geolocation and see which ones are open now.", path: "/pharmacies" },
    { icon: <Clock className="h-6 w-6" />, title: "Instant Reservations", desc: "Reserve medicines directly through the platform and pick them up at your convenience.", path: "/search" },
    ...
    { icon: <Smartphone className="h-6 w-6" />, title: "Mobile Optimized", desc: "Access the entire MediLinkRx platform seamlessly from your phone, tablet, or desktop." },
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div className="text-center max-w-3xl mx-auto mb-16">
        ...
        <p className="text-lg text-slate-600">MediLinkRx bridges the gap between patients and pharmacies with state-of-the-art tools designed for speed, reliability, and convenience.</p>
      </div>
      
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        {features.map((f, i) => {
          const CardContent = (
            <div className="bg-white p-8 rounded-2xl shadow-soft border border-slate-100 hover:shadow-xl hover:-translate-y-1 transition-all duration-300 h-full cursor-pointer">
